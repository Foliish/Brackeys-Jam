class_name WebSocketNetworkManager
extends NetworkAdapter

## Singleton network manager for WebSocket multiplayer connections and RPC messaging.
...
static var _instance: WebSocketNetworkManager = null
...
static func get_instance() -> WebSocketNetworkManager:
...
static func start_server(port: int = DEFAULT_PORT, bind_ip: String = "*", tls_server_options: TLSOptions = null) -> bool:
	if is_instance_valid(_instance):
		_instance._stop_internal()

	var inst := WebSocketNetworkManager.new()
	inst.name = "WebSocketNetworkManager"
	var tree := Engine.get_main_loop() as SceneTree
	if tree and tree.root:
		tree.root.add_child(inst)
	else:
		push_error("[WebSocketNetworkManager] Failed to access SceneTree root.")
		return false

	_instance = inst
	return inst._start_server_internal(port, bind_ip, tls_server_options)


# Connects to the server using IP address/URL and port (creates singleton instance)
static func connect_to_server(ip_or_url: String, port: int = DEFAULT_PORT, tls_client_options: TLSOptions = null) -> bool:
	if is_instance_valid(_instance):
		_instance._stop_internal()
	...
	_instance = inst
	return inst._connect_to_server_internal(ip_or_url, port, tls_client_options)
...
func _start_server_internal(port: int, bind_ip: String = "*", tls_server_options: TLSOptions = null) -> bool:
	peer = WebSocketMultiplayerPeer.new()
	var err := peer.create_server(port, bind_ip, tls_server_options)

	if err != OK:
		push_error("Failed to start WebSocket server: %d" % err)
		_stop_internal()
		return false
	...
	print("[NET-WS] WebSocket server started on port: ", port)
	return true


func _connect_to_server_internal(ip_or_url: String, port: int = DEFAULT_PORT, tls_client_options: TLSOptions = null) -> bool:
	connecting.emit()

	var target_url := ip_or_url
	if not (target_url.begins_with("ws://") or target_url.begins_with("wss://")):
		if ":" in target_url:
	...
	peer = WebSocketMultiplayerPeer.new()
	var err := peer.create_client(target_url, tls_client_options)

	if err != OK:
		push_error("Failed to connect to WebSocket server at: " + target_url)
	...
	multiplayer.multiplayer_peer = peer
	_register_multiplayer_signals()
	print("[NET-WS] Connecting to WebSocket server: ", target_url)
	return true
...
func _on_peer_connected(id: int):
	print("[NET-WS] Peer connected with ID: ", id)
	peer_connected.emit(id)


# Called when a peer disconnects
func _on_peer_disconnected(id: int):
	print("[NET-WS] Peer disconnected with ID: ", id)
	peer_disconnected.emit(id)


# Called on client when connection to server succeeds
func _on_connected():
	print("[NET-WS] Connection to server successfully established")
	connected_to_server.emit()


# Called on client when connection to server fails
func _on_connection_failed():
	push_error("[NET-WS] Failed to connect to server")
	connection_failed.emit()


# Called when disconnected from server
func _on_server_disconnected():
	print("[NET-WS] Disconnected from server")
