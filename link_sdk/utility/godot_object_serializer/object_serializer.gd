## Main class for godot-object-serializer. Holds the script registry.
## [url]https://github.com/Cretezy/godot-object-serializer[/url]
class_name ObjectSerializer

## Field containing type in serialized object values. Modification is not recommended.
## It should be unique to avoid conflicts with keys in objects/dictionaries.
## Can be changed, but must be configured before serialization or deserialization starts.
static var type_field := "._type"

## Field containing constructor arguments in serialized object values. Modification is not recommended.
## It should be unique to avoid conflicts with keys in objects.
## Can be changed, but must be configured before serialization or deserialization starts.
static var args_field := "._"

## Prefix for object types stored in [member type_field]. Modification is not recommended.
## Must be unique to avoid conflicts with built-in type names.
## Can be changed, but must be configured before serialization or deserialization starts.
...
## If [member require_export_storage] is true, serializing variables will also require [@GlobalScope.PROPERTY_USAGE_STORAGE].
## This can be set for variables using the [annotation @GDScript.@export_storage] annotation. Example: [code]@export_storage var name: String[/code]
...
## Registers a script (object type) for serialization/deserialization. All custom types (including nested) must be registered [b]before[/b] using this library.
## The [param name] parameter can be empty if the script uses [code]class_name[/code] (for example, [code]ObjectSerializer.register_script("", Data)[/code]), but it is usually better to provide the name explicitly.
...
## Registers multiple scripts (object types) from a dictionary.
...
class _ScriptRegistryEntry:
	...
	## If [param json_keys] is enabled, attempts to convert string keys of type int/float/bool to their respective types.
					...
					if json_keys and instance[key].is_typed_key():
						match instance[key].get_typed_key_builtin():
							...
							_:
								assert(
									false,
									"Attempted to deserialize JSON into dictionary with non-primitive (non String/int/float/bool) keys"
								)
					else:
						instance[key].assign(key_value)
				TYPE_ARRAY:
